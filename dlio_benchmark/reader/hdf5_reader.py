...
class HDF5Reader(FormatReader, _LocalFSIterableMixin):
    """
    Reader for HDF5 files.

    Uses _LocalFSIterableMixin to prefetch all assigned files in parallel
    before the iteration loop. h5py decode is skipped because only the
    ...
    def close(self, filename):
        # Nothing to close — we only stored a byte count, not an h5py handle.
        pass
